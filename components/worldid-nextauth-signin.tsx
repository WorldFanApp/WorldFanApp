...
import { Shield, Check, AlertCircle, Globe, LogOut, User, Loader2 } from "lucide-react"
...
export function WorldIDNextAuthSignIn({ onSuccess, onDeveloperMode }: WorldIDNextAuthSignInProps) {
  ...
  useEffect(() => {
    if (session?.user?.worldcoin) {
      console.log("NextAuth session established with World ID:", session.user.worldcoin)
  ...
  const handleSignIn = async () => {
    ...
    try {
      console.log("Starting NextAuth World ID sign-in...")
    ...
    return (
      <>
        ...
        <CardContent className="text-center">
          <div className="text-sm text-gray-600">Please wait while we verify your session...</div>
        </CardContent>
      </>
    )
  }
    ...
    return (
      <>
        <CardHeader className="text-center">
          ...
          <CardDescription>You're successfully authenticated with World ID via NextAuth</CardDescription>
        </CardHeader>
          ...
          <div className="bg-green-50 p-4 rounded-lg">
            ...
            <div className="text-sm text-green-800 space-y-1">
              ...
              {session.user.worldcoin && (
                <>
                  <p>
                    <strong>Verification Level:</strong> {session.user.worldcoin.verification_level || "Verified"}
                  </p>
                  <p>
                    <strong>World ID:</strong> {session.user.worldcoin.nullifier_hash?.slice(0, 20)}...
                  </p>
                </>
              )}
              <p>
                <strong>Provider:</strong> NextAuth + World ID
              </p>
            </div>
          </div>
  ...
  return (
    <>
      <CardHeader className="text-center">
        ...
        <CardDescription>Secure authentication using NextAuth.js and World ID OpenID Connect</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="grid gap-4">
          <div className="flex items-center gap-3 p-3 bg-green-50 rounded-lg">
            ...
            <div>
              <p className="font-medium text-green-900">NextAuth Integration</p>
              <p className="text-sm text-green-700">Industry-standard OAuth2/OpenID Connect flow</p>
            </div>
          </div>
          <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
            <Check className="w-5 h-5 text-blue-600" />
            <div>
              <p className="font-medium text-blue-900">World ID Verified</p>
              <p className="text-sm text-blue-700">Proof of unique human identity</p>
            </div>
          </div>
          <div className="flex items-center gap-3 p-3 bg-purple-50 rounded-lg">
            <User className="w-5 h-5 text-purple-600" />
            <div>
              <p className="font-medium text-purple-900">Secure Sessions</p>
              <p className="text-sm text-purple-700">JWT-based session management</p>
            </div>
          </div>
        </div>
